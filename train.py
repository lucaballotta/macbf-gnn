...
def main():
    ...
    cbf_controller = controller.Controller(in_dim=4)
    cbf_certificate = cbf.CBF(in_dim=4)
    ...
    for _ in tqdm(range(TRAIN_STEPS)):
        ...
        for _ in tqdm(range(INNER_LOOPS)):
            states_trajectory.append(np.copy(states_curr))
            goals_trajectory.append(np.copy(goals_curr))

            # compute the control input using the trained controller
            actions_curr = cbf_controller.forward(states_curr, goals_curr)
            if np.random.uniform() < ADD_NOISE_PROB:
                noise = np.random.normal(size=np.shape(actions_curr)) * NOISE_SCALE
                actions_curr = actions_curr + noise

            # simulate the system for one step
            states_curr = states_curr + dynamics(states_curr, actions_curr) * TIME_STEP
            actions_trajectory.append(np.copy(actions_curr))
        ...
        states_trajectory = torch.from_numpy(states_trajectory)
        goals_trajectory = torch.from_numpy(goals_trajectory)
        actions_trajectory = torch.from_numpy(actions_trajectory)

        # compute loss for batch of trajectory states
        h_trajectory = cbf_certificate.forward(states_trajectory)
